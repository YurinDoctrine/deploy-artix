...
pacman -Sy --noconfirm --disable-download-timeout acpid-$MY_INIT alsa-utils bluez-$MY_INIT doas gtk-engines gtk-engine-murrine haveged-$MY_INIT kitty libva-mesa-driver macchanger mesa mesa-vdpau openbox pipewire pipewire-alsa rsync thermald-$MY_INIT tor-$MY_INIT torsocks unzip vim wayland wget wireplumber wpa_supplicant xdg-desktop-portal-gtk xdg-utils xdg-user-dirs xorg xorg-xinit xterm
