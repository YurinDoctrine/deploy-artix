...
if [ "$ENCRYPTED" = "y" ]; then
  ...
  sed -i -e "s/GRUB_CMDLINE_LINUX_DEFAULT=.*/GRUB_CMDLINE_LINUX_DEFAULT=\"cryptdevice=UUID=$DRIVE_UUID:root root=UUID=$ROOT_UUID quiet rootfstype=ext4,btrfs,xfs,f2fs biosdevname=0 nowatchdog noautogroup noresume default_hugepagesz=1G hugepagesz=1G zswap.enabled=1 zswap.compressor=lz4 zswap.max_pool_percent=20 zswap.zpool=zsmalloc workqueue.power_efficient=1 pci=pcie_bus_perf,noaer rd.plymouth=0 plymouth.enable=0 plymouth.ignore-serial-consoles logo.nologo consoleblank=0 vt.global_cursor_default=0 rd.systemd.show_status=auto loglevel=0 rd.udev.log_level=0 udev.log_priority=0 audit=0 nosoftlockup=0 selinux=0 enforcing=0 mce=off no_timer_check skew_tick=1 clocksource=tsc tsc=perfect rcu_nocb_poll irqpoll threadirqs irqaffinity=0 noirqdebug idle=nomwait noatime boot_delay=0 io_delay=none rootdelay=0 elevator=noop init_on_alloc=0 init_on_free=0 mitigations=off ftrace_enabled=0 fsck.repair=no fsck.mode=skip cgroup_disable=memory cgroup_no_v1=all\"/g" /etc/default/grub
  sed -i -e '/GRUB_ENABLE_CRYPTODISK=y/s/^#//g' /etc/default/grub
else
  sed -i -e 's/GRUB_CMDLINE_LINUX_DEFAULT=.*/GRUB_CMDLINE_LINUX_DEFAULT="quiet rootfstype=ext4,btrfs,xfs,f2fs biosdevname=0 nowatchdog noautogroup noresume default_hugepagesz=1G hugepagesz=1G zswap.enabled=1 zswap.compressor=lz4 zswap.max_pool_percent=20 zswap.zpool=zsmalloc workqueue.power_efficient=1 pci=pcie_bus_perf,noaer rd.plymouth=0 plymouth.enable=0 plymouth.ignore-serial-consoles logo.nologo consoleblank=0 vt.global_cursor_default=0 rd.systemd.show_status=auto loglevel=0 rd.udev.log_level=0 udev.log_priority=0 audit=0 nosoftlockup=0 selinux=0 enforcing=0 mce=off no_timer_check skew_tick=1 clocksource=tsc tsc=perfect rcu_nocb_poll irqpoll threadirqs irqaffinity=0 noirqdebug idle=nomwait noatime boot_delay=0 io_delay=none rootdelay=0 elevator=noop init_on_alloc=0 init_on_free=0 mitigations=off ftrace_enabled=0 fsck.repair=no fsck.mode=skip cgroup_disable=memory cgroup_no_v1=all"/' /etc/default/grub
fi
