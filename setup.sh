...
if [ "$MY_FS" = "btrfs" ]; then
  if [ "$ENCRYPTED" = "y" ]; then
    basestrap /mnt base $MY_INIT elogind-$MY_INIT efibootmgr grub $ucode dhcpcd-$MY_INIT connman-$MY_INIT btrfs-progs cryptsetup cryptsetup-$MY_INIT
  else
    basestrap /mnt base $MY_INIT elogind-$MY_INIT efibootmgr grub $ucode dhcpcd-$MY_INIT connman-$MY_INIT btrfs-progs
  fi
else
  if [ "$ENCRYPTED" = "y" ]; then
    basestrap /mnt base $MY_INIT elogind-$MY_INIT efibootmgr grub $ucode dhcpcd-$MY_INIT connman-$MY_INIT cryptsetup cryptsetup-$MY_INIT
  else
    basestrap /mnt base $MY_INIT elogind-$MY_INIT efibootmgr grub $ucode dhcpcd-$MY_INIT connman-$MY_INIT
  fi
fi
