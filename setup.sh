...
if [ "$MY_FS" = "btrfs" ]; then
  if [ "$ENCRYPTED" = "y" ]; then
    basestrap /mnt base $MY_INIT elogind-$MY_INIT efibootmgr dbus-$MY_INIT dhcpcd-$MY_INIT grub $UCODE wpa_supplicant-$MY_INIT btrfs-progs cryptsetup cryptsetup-$MY_INIT
  else
  ...
  if [ "$ENCRYPTED" = "y" ]; then
    basestrap /mnt base $MY_INIT elogind-$MY_INIT efibootmgr dbus-$MY_INIT dhcpcd-$MY_INIT grub $UCODE wpa_supplicant-$MY_INIT cryptsetup cryptsetup-$MY_INIT
  else
