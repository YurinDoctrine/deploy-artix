...
confirm_password() {
  ...
  until [ "$pass1" = "$pass2" ] && [ "$pass2" ]; do
    echo -e "%s: " "$1" >&2 && read -rp ">_:" pass1
    echo -e "Confirm %s: " "$1" >&2 && read -rp ">_:" pass2
  done
  stty echo
  echo -e "$pass2"
}
...
# Load keymap
echo -e "Load keymap (e.g. us): " && read -rp ">_:" $MY_KEYMAP && sudo loadkeys $MY_KEYMAP
...
while :; do
  ...
  echo -e "Disk to install to (e.g. /dev/Xda): " && read -rp ">_:" MY_DISK
  [ -b "$MY_DISK" ] && break
done
...
until (echo "$SWAP_SIZE" | grep -Eq "^[0-9]+$") && [ "$SWAP_SIZE" -gt 0 ] && [ "$SWAP_SIZE" -lt 97 ]; do
  echo -e "Size of swap partition in GiB (Default: 4GB): " && read -rp ">_:" SWAP_SIZE
  [ ! "$SWAP_SIZE" ] && SWAP_SIZE = 4
done

# Choose filesystem
until [ "$MY_FS" = "btrfs" ] || [ "$MY_FS" = "ext4" ]; do
  echo -e "Filesystem (btrfs/ext4): " && read -rp ">_:" MY_FS
  [ ! "$MY_FS" ] && MY_FS = "btrfs"
done
...
# Encrypt
echo -e "Encrypt? (y/N): " && read -rp ">_:" ENCRYPTED
...
until [ -f /usr/share/zoneinfo/"$REGION_CITY" ]; do
  printf "Region/City (e.g. 'America/Denver'): " && read -rp ">_:" REGION_CITY
  [ ! "$REGION_CITY" ] && REGION_CITY = "America/Denver"
done

# Host
while :; do
  printf "Hostname: " && read -rp ">_:" MY_HOSTNAME
  [ "$MY_HOSTNAME" ] && break
done
